...
{
    [TestFixture]
    public class DefaultWindowsLiveLoginRequestWriterTester : InteractionContext<DefaultWindowsLiveLoginRequestWriter>
    {
        private string theTag;
        private WindowsLiveSignIn theSignIn;
        ...
        {
            theUrl = "login/test";
            theRequest = new WindowsLiveLoginRequest();
            theSignIn = new WindowsLiveSignIn();

            MockFor<IFubuRequest>().Stub(x => x.Get<WindowsLiveSignIn>()).Return(theSignIn);
        ...
        {
            theTag.ShouldEqual("<a href=\"{0}\">{1}</a>".ToFormat(theUrl, WindowsLiveLoginKeys.LoginWithWindowsLive.ToString()));
        }
    }
}