...
{
    [TestFixture]
    public class when_handling_a_successful_facebook_response : InteractionContext<OAuth2ResponseHandler<FacebookLoginRequest>>
    {
        private FacebookLoginRequest theRequest;

        protected override void beforeEach()
        {
            theRequest = new FacebookLoginRequest { Url = "hello/there" };

            MockFor<IFubuRequest>().Stub(x => x.Get<FacebookLoginRequest>()).Return(theRequest);
    ...
    [TestFixture]
    public class when_handling_a_failed_facebook_response : InteractionContext<OAuth2ResponseHandler<FacebookLoginRequest>>
    {
        private FacebookLoginRequest theRequest;
