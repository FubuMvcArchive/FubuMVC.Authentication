...
{
    [TestFixture]
    public class when_handling_a_successful_github_response : InteractionContext<OAuth2ResponseHandler<GithubLoginRequest>>
    {
        private GithubLoginRequest theRequest;

        protected override void beforeEach()
        {
            theRequest = new GithubLoginRequest { Url = "hello/there" };

            MockFor<IFubuRequest>().Stub(x => x.Get<GithubLoginRequest>()).Return(theRequest);
    ...
    [TestFixture]
    public class when_handling_a_failed_github_response : InteractionContext<OAuth2ResponseHandler<GithubLoginRequest>>
    {
        private GithubLoginRequest theRequest;
